...
- 공개 종료 시각이 지난 수업은 `완료된 교육 세션`으로 자동 분리됩니다.
- NAS 폴더를 주기적으로 확인하고, 검증과 임시 빌드에 성공한 변경만 자동 반영합니다.
- 운영 중 발견된 잘못된 JSON, 누락된 인증 파일, 위험한 경로, 심볼릭 링크는 기존 사이트를 중단하지 않고 무시합니다. 최초 시작 시 설정이 잘못되면 안전하게 시작을 중단합니다.
...
- `availableUntil`이 지나면 홈페이지와 수업 목록의 `완료된 교육 세션` 영역으로 이동합니다.
...
| `CONTENT_AUTO_REFRESH` | `true` | NAS 콘텐츠 변경 자동 확인 사용 여부 |
| `CONTENT_POLL_SECONDS` | `60` | NAS 폴더 변경 확인 주기(10~3600초) |
| `CONTENT_SETTLE_SECONDS` | `15` | 업로드 완료를 기다리는 안정화 시간(5~600초) |
...
- 검증 도중 파일이 다시 바뀜: 업로드 중으로 판단하고 다음 확인까지 기다립니다.
- `availableFrom` 또는 `availableUntil` 경계를 지남: 파일 변경이 없어도 상태 변경으로 감지하여 목록을 갱신합니다.

대용량 ZIP은 임시 확장자(예: `.uploading`)로 업로드한 뒤 최종 이름으로 변경하면 중간 파일이 목록에 노출될 가능성을 더 줄일 수 있습니다. 자동 갱신을 원하지 않으면 `CONTENT_AUTO_REFRESH=false`로 설정하고 기존처럼 직접 재시작할 수 있습니다.
