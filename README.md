...
- 자주 사용하는 ZIP 파일을 `전체 다운로드` 버튼으로 노출할 수 있습니다.
  ...
  "featuredDownloads": [
    {
      "label": "전체 강의자료 받기",
      "type": "materials",
      "path": "all-materials.zip"
    },
    {
      "label": "전체 실습코드 받기",
      "type": "code",
      "path": "practice-code.zip"
    }
  ]
}
```
...
- `featuredDownloads`: 선택 항목입니다. `type`은 `materials` 또는 `code`, `path`는 해당 폴더 내부의 상대 경로입니다.
- `docs`에는 Markdown 문서가 하나 이상 필요합니다.
- `materials`와 `code`의 모든 파일도 수업 페이지에서 개별 다운로드할 수 있습니다.
- 여러 파일의 일괄 다운로드는 NAS에서 미리 ZIP으로 묶은 뒤 `featuredDownloads`에 지정하는 방식을 권장합니다.
