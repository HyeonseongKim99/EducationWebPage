...
async function fingerprint(courses, now) {
  const {stdout} = await execFileAsync(process.execPath, [watcher, '--fingerprint'], {
    cwd: projectRoot,
    env: {...process.env, COURSES_PATH: courses, CONTENT_FINGERPRINT_NOW: String(now)},
  });
  return stdout.trim();
}
...
test('NAS 파일 변경과 공개 상태 변경을 감지한다', async (t) => {
  ...
  const before = await fingerprint(courses, Date.parse('2998-01-01T00:00:00+09:00'));
  await fs.writeFile(path.join(course, 'docs', 'intro.md'), '변경된 내용입니다');
  const afterFile = await fingerprint(courses, Date.parse('2998-01-01T00:00:00+09:00'));
  assert.notEqual(afterFile, before);

  const afterDeadline = await fingerprint(courses, Date.parse('3000-01-01T00:00:00+09:00'));
  assert.notEqual(afterDeadline, afterFile);

  await fs.writeFile(path.join(course, 'course.json'), '{잘못된 JSON');
  const invalid = await fingerprint(courses, Date.parse('3000-01-01T00:00:00+09:00'));
